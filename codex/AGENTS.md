...
- `$glaze` is available for explicit invocation and selective implicit escalation. Implicit availability is not a default invocation. Invoke it only after an incumbent is concrete and before adjudicating that incumbent when at least one escalation pressure is live: a contradiction; repeated same-surface repair or review accretion; a high-regret architectural commitment; a plausible owner, model, or representation error; or a coherent but merely adequate local optimum with a materially different construction still plausible. Mere substantiveness or consequentiality is not a trigger.
- Before invocation, freeze the decision surface: accepted intent, owning workflow, incumbent, target observation or acceptance criterion, current evidence, and falsifier. If an explicit invocation cannot be bounded, return `blocked` and name the missing binding. If an implicit invocation cannot be bounded, do not invoke it. Proxy evidence cannot satisfy the original observation.
- Run the canonical Glaze line exactly once per unchanged decision surface. Return one generative result: `material-reframe`, `no-material-reframe`, or `blocked`. A material reframe must change a governing model, owner, representation, admitted domain, search space, proof obligation, or falsifier; rhetoric, renamed summaries, and ungrounded novelty are `no-material-reframe`.
- Glaze is generative, not authoritative. It may generate a candidate but cannot select it, grant mutation authority, widen accepted scope, or authorize closure. The owning workflow separately records `adopted`, `modified`, `rejected`, or `pending` and owns every subsequent effect. Require evidence bound to the original observation before actuation or closure.

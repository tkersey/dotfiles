...
- `$metanoetic` is a selective, one-pass generative interrupt over a concrete incumbent, not a default pass. Invoke it only when a contradiction, repeated same-surface repair or accretion, a high-regret commitment, a plausible owner, model, or representation error, or a coherent but merely adequate local optimum makes a materially different candidate plausible. Mere substantiveness or consequentiality is not a trigger.
- Within `$actuating`, an initial Construction is eligible after the first `$universalist` nomination only when that nomination would establish a high-regret or difficult-to-reverse commitment, remains a coherent but merely adequate local optimum, and a materially different Construction is still plausible. Mere architectural consequence, novelty, or change size is not enough.
- Bind enough of the current task to compare the incumbent and any challenger against the original objective, observation, acceptance criterion, or discriminator. For implicit use, skip Metanoetic when that comparison cannot be bounded. For explicit use, resolve the incumbent from the current context and return `blocked` only when no concrete antecedent or comparison criterion exists.
- Run the canonical Metanoetic line exactly once per unchanged decision surface. Metanoetic generates candidates only and supplies no new premise or authority. The receiving workflow owns materiality, admissibility, disposition, evidence, selection, mutation, and closure through its native fields and contracts; do not create a universal Metanoetic result taxonomy or observation artifact.
- When an owning workflow already specifies a Metanoetic gate, satisfy that gate rather than adding another pass. A later material change to the bound objective, incumbent, evidence, or discriminator may create a new decision surface.

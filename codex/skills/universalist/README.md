...
This archive is laid out for extraction at the repository root. Replace the old skill directory before extracting so stale files cannot survive:

```bash
rm -rf codex/skills/universalist
unzip -o universalist-17.1.0-compiler-dropin.zip -d .
chmod +x codex/skills/universalist/scripts/*.sh codex/skills/universalist/scripts/*.py
cd codex/skills/universalist
./scripts/check_universalist_replacement.sh
```

The shipped validation surface is deliberately small: one package doctor plus the retained mechanics smoke entrypoints. The large development test corpus used to build this release is not included in the drop-in.
...
Use `$universalist` for the full boundary workflow. Detailed Kan extension/lift, Freyd/AFT, pullback/pushout, comonads-as-spaces, Yoneda/Coyoneda, density/codensity, categorical-data, graph-rewrite, description-composition, contextual-morphism, and defunctionalization mechanics live inside `references/mechanics/` and `scripts/emit_mechanics_report.sh`; load them only after the boundary pass justifies escalation.

## Evidence-bound Universal Problem shadow

`universal-problem/v6` preserves the categorical corpus but uses it as a hidden nomination and proof-obligation optimizer rather than a user-facing pattern menu.

The executable compiler is **experimental and optional**. It is a deterministic evidence-shape checker and theorem-card nominator, not a substitute for root-agent architectural judgment. The same Universal Problem protocol may be run manually from the skill instructions when the compiler would add ceremony rather than clarity.

```text
ordinary repository-native candidate
  -> boundary disposition
  -> evidence-bound comparison universe
  -> one architectural axis and one typed hole
  -> exact requirement witnesses and candidate proof profiles
  -> evidence-bound rejection or contradiction of competing alternatives
  -> nomination, ordinary fallback, underdetermination, conflict, or evidence debt
  -> root adjudication
  -> repository-native lowering or obstruction
```

A single seam may need several independent axes—such as data shape, ordered effects, description composition, context action, and locality. Compile those as separate linked Universal Problem packets under the same root plan. The compiler rejects cross-axis signals rather than selecting one global theorem-card winner and silently dropping an obligation.

Nomination mode discovers compatible theorem cards; verification mode names one exact `verification_target`. The compiler validates and nominates; the root workflow adjudicates and owns the final route, implementation authorization, and Ledger/Seq receipt. Material delta is derived from exact requirement-level candidate witnesses and cannot be supplied by the caller. A nominated universal construction also needs evidence for existence, commutation/preservation, competitor mediation, canonicality/uniqueness-up-to, effectivity, and a falsifier. Obstruction needs nonexistence, counterexample, stability, effectivity, falsifier, and reopening evidence.

Unknown signals, unknown prerequisite keys, unrelated facts, truthy prose, missing proof obligations, and unresolved alternatives fail closed. A typed rejection must name its basis and evidence. If requirements remain unmet with no classified candidate, the result is `unclassified-hole`; if every classified candidate is contradicted or rejected, the result is `no-admissible-construction`. Neither result nominates an ordinary route. Signal count, evidence count, diagnostic priority, and registry order cannot break ties. Missing evidence is an epistemic block, not an obstruction. A complete obstruction and a complete constructive candidate produce an explicit evidence conflict. A bounded approximation additionally names its loss boundary and refinement condition.

Ordinary routing follows the boundary disposition: only an already exact preserved seam yields `UNI-PRESERVE`; an introduced, changed, repaired, or removed seam with a sufficient ordinary solution yields `UNI-ORDINARY`.
...
Derived expert mechanics—category pivots, spatial description products, spatial context framing, Day-center/strong-context guards, Cayley-style representations, and primitive presentation—compile as linked packets or support guards rather than global peer winners.

Default output uses plain engineering language. Add `--explain-theory` only for mathematical audit; it reveals the expert name, universal-property guidance, and linked theorem references without changing the nomination. See `references/universal-problem-ir.md`.

## Ledger-addressed plans

Implementation tracks allocate a fresh progress artifact through the
`skills-zig` Ledger 0.10.4 or newer with Skills Seq 0.3.51 or newer:

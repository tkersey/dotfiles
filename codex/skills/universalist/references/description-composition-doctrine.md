...
Density and Day solve different problems:
...
For subbases `P1` and `P2`, spatial convolution may be generated by external-product patches:

```text
(U,V) |-> P1(U) x P2(V)
```

This can combine dependency, capability, evidence, or ownership locality without merely concatenating neighborhood lists. Require an effective product of halos, label translation, continuous projections, and a resource bound.

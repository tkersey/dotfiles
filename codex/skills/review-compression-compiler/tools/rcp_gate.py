...
def main(argv: list[str]) -> int:
    ...
    if has(r"active_exclusion_match\s*:\s*yes", text) and has(r"handoff_allowed\s*:\s*yes", text):
        print("RCP gate: FAIL")
        print("active exclusion match cannot allow handoff")
        return 1

    if has(r"prior_decision_invalidated\s*:\s*yes", text) and has(r"negative_capture_candidate\s*:\s*yes", text) and has(r"query_status\s*:\s*not-run", text):
        print("RCP gate: FAIL")
        print("falsified route requires negative-ledger query/map")
        return 1

> Compatibility boundary: this is the frozen `legacy-actuating-v1` Actuating
> workflow. Use it only for a goal already marked with that protocol during the
> Phase-4/5 compatibility window. It is never an input, authority source, or
> review-credit source for `artifact-kernel-v1`.
...
Read [legacy-live-semantics.yaml](legacy-live-semantics.yaml) for canonical
vocabulary and [legacy-closure.md](legacy-closure.md) for the terminal projection.
...
Read [legacy-closure.md](legacy-closure.md), then run:
...
- [legacy-live-semantics.yaml](legacy-live-semantics.yaml) — canonical worlds,
  transitions, ownership, and laws.
- [review-policy.md](review-policy.md) — lens selection, exact
  request binding, lane accounting, and invalidation.
- [review-resolution.md](review-resolution.md) — cumulative owner
  synthesis, per-class refinement, strategy rules, and semantic balance.
- [legacy-closure.md](legacy-closure.md) — kernel terminal projection and lifecycle
  handoff.
- [legacy-decision-contract.yaml](legacy-decision-contract.yaml) — trigger and

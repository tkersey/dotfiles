...
not load a stored architecture.
...
```text
sanctioned inputs and boundaries
owner set and authority
representations and state dimensions
...
Historical analyses are hypotheses unless the current tree and owner evidence
still support them.
...
4. Build an observational class through `$review-fold`.
5. Derive the ordinary counterexample theory.
6. Decide whether the incumbent already excludes the family through the correct
   owner and sanctioned paths.

An isolated implementation deviation inside an already-correct owner remains a
realization repair.
...
The incumbent is closed only when:

```text
every concrete violation is reflected into Phi
every admitted behavior maps outside Phi
...
the canonical owner and enforcement mechanism already exist
no new state, law, representation, recovery mode, or escape path is required
```

A convenient place for another validator is not closure.
...
2. Finalize one current theory and falsifier.
3. Ask `$universalist` for repository-native candidates.
...
7. Establish the correctness Pareto frontier.
8. Among safety- and Goal-equivalent candidates, select the least costly
...
locates the failure in realization/proof and the new proof covers the failed
case plus predicted siblings.
...
selected Theta / owner / cut / target
...
The Working Set grants no authority and has no schema, identity, store, or
migration. Git is the realized construction.
...
- a PR explanation;
- a genuine long-lived ADR when warranted.

Do not create an Actuating-private source of truth.

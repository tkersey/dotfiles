...
```text
Close(G, K6, Q*, E, R, current_subject)
  -> continue | ready-to-ship | complete | blocked
```

`G` is the current Goal Contract. `K6` is the current
`construction-contract/v6`. `Q*` is the complete applicable Counterexample
...
1. `K6` references the current `G`, is the current Construction, and its base
   subject plus recorded effect chain yields the exact clean live subject.
2. `K6.counterexample_class_refs` is the complete set of currently applicable

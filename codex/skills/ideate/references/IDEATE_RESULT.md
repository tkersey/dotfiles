# Ideate Result Receipt (IDR-v2)

`IDR-v2` is a compact, auditable receipt that proves whether `$ideate` actually performed the important gates. It is not an implementation artifact and does not expose private reasoning.
...
- `audit-only` may skip Metanoetic/ASI if it is only reporting signals.

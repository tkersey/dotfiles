...
`the finding is valid`, `this finding is valid too`, `accepted code-change
liabilities`, `the owner fix is`, `owner-correct fix is`, `the clean fix is`,
`clean streak stays at 0`, or `streak remains 0`. Those phrases accept
liability or reset review accounting; they are not RF receipts. Emit the fresh
RF-v1.3 compact/full receipt before describing the fix path, resolution node, or
next mutation.
...
11. Treat single or small-batch CAS prose like `CAS attempt found a P1/P2`, `CAS attempt returned P1/P2 findings`, `CAS found valid items`, `CAS found one remaining valid P1/P2`, `the finding is valid`, `this finding is valid too`, `accepted code-change liabilities`, `the owner fix is`, `owner-correct fix is`, `the clean fix is`, `clean streak stays at 0`, or `streak remains 0` as receipt-triggering folds before any repair path is described.

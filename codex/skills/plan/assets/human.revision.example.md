...
A-TEST (implementation owner): In tests/unit/test_durations.py, add tests that -1 and -99
raise ValueError and that 0, +3, and whitespace-padded 5 retain results 0, 3, and 5.
Run `python -m unittest tests.unit.test_durations`; the negative tests must expose the
...
Source: the same required behavior as revision 1, restated completely here.
The fixture now locates focused tests at tests/unit/test_durations.py. A-TEST and
its commands are revised; A-FIX and its authority are preserved. Old focused-test
command evidence is invalidated; rerun the revised focused and final proof. No real commit or test execution

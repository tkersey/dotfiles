...
- Execution-efficacy trim gate: in `fast` and `balanced`, a section earns expanded prose only when it changes implementer behavior: implementation steps, interfaces/contracts, data boundaries, acceptance checks, rollout/rollback, risk controls, ownership, or unresolved decisions. Otherwise collapse to lint-safe schema-minimal marker rows, for example `scope_change=none; reason=no material scope change; approved_by=n/a`.
- Duplication-collapse gate: explain each material decision once, normally in `Decision Log`. Other sections should state only their execution-specific effect (affected API, test, owner, rollout step, rollback trigger, etc.) and may reference stable decision ids; do not re-narrate the same rationale across audit/support sections.
- Greenfield bootstrap gate: when the user starts a new project, product, or major feature from sparse input and no mature source plan exists, run a lean bootstrap pass before drafting: infer or lock (1) goal/why, (2) primary users and workflows, (3) non-goals, (4) stack/runtime/integration surfaces, (5) architecture pattern, (6) deployment/operability constraints, and (7) success bar.
- Bootstrap routing gate: do not add a default `Greenfield Bootstrap` output section. Route bootstrap decisions into existing sections: goal/why and success bar into `Summary` and tests/acceptance; workflows into data flow and requirement-to-test traceability; stack/runtime/integrations into interfaces/types/APIs impacted and `Decision Log`; architecture/deployment constraints into data flow, rollout/monitoring, rollback, and `Implementation Brief`; uncertainties into assumptions/defaults or `Open Questions`.
- Bootstrap thrift gate: include only bootstrap content that changes implementation choices, acceptance checks, rollout/rollback, or handoff. Omit generic product narrative, speculative roadmap features, and stack recommendations that are not justified by the user goal or current research.
...
- For sparse greenfield inputs, ask at most one blocking bootstrap question only when a missing answer would materially change stack/runtime, architecture pattern, primary workflow, data boundary, or deployment target. Otherwise choose a recommended default, record provenance, and continue.
...
- Greenfield bootstrap routing is correct: for sparse new project/product/feature inputs, the plan captures goal/why, primary workflows, stack/runtime/integrations, architecture pattern, deployment constraints, and success bar in existing execution sections; it does not emit a standalone `Greenfield Bootstrap` section unless explicitly requested.
- Response trimming is efficacy-based and lint-safe: in `fast` and `balanced`, audit/support sections are compact unless expanded content changes implementation, verification, rollout/rollback, risk handling, ownership, or an unresolved decision; empty support sections use schema-minimal marker rows instead of prose-only placeholders, except `Open Questions` may be `None`/`n/a`.

...
description: "Drive exhaustive build-review-improve-verify loops to a material fixed point across accretive-implementer, adversarial-reviewer, review-adjudication, verification-closure, default negative-ledger, and learnings-backed evidence. Trigger when coding needs de novo re-litigation, proof-gated closure, repeated review/fix loops, negative-evidence pruning, optional read-only specialists, soundness/foot-gun/complexity routing, a mandatory pre-closure one-change challenge, and closure handoff. Use for harden this patch exhaustively, address PR reviews to closure, keep re-reviewing, find all impactful changes, adjudicate then implement, drive to fixed point, or avoid repeating failed routes. Do not use for trivial one-step tasks or when user wants one narrow phase."
---

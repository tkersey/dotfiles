...
## Resolution Warrants schema

Resolution Warrants are the handoff permission ledger. Use one row per raw claim
that appears in Resolve Selection. Do not collapse multiple raw claims unless the
merged warrant explicitly lists every claim id in scope.
...
Every implementation handoff must be backed by a `mutate-code` warrant. Every
validation-only route must be backed by `add-validation-only`. Every proof-only
thread cleanup must be backed by `resolve-thread`.

...
2. Coordinator-only `bd`:
   - Sub-agents run in jj workspaces and **do not** perform `bd` writes.
     Read-only access uses `bd --readonly --sandbox` (see contract below).
   - The coordinator performs all bead updates, comments, and status changes.
3. Agree on a concurrency cap (max agents per wave).

## Sub-agent bd read-only contract

Sub-agents may only read beads with:
...
- Global flag allowlist: `-h/--help`, `--json`, `-q/--quiet`, `-v/--verbose`.
  ...
  `--no-*`, `--lock-timeout`). Use `--allow-stale` only when strictly needed.
...
Examples (allowed):
- `bd --readonly --sandbox show bd-123 --thread`
- `bd --readonly --sandbox list --status in_progress --pretty`
- `bd --readonly --sandbox dep tree bd-123`
- `bd --readonly --sandbox mol progress bd-mol-1`
- `bd --readonly --sandbox swarm status bd-epic-1`

Examples (rejected):
- `bd --readonly --sandbox dep bd-123 --blocks bd-456`
- `bd --readonly --sandbox comments add bd-123 "nope"`
- `bd --readonly --sandbox update bd-123 --status in_progress`
...
- The sub-agent prompt must include bead context (copied from `bd show`) and the
  rule: **do not run `bd` writes; use `bd --readonly --sandbox` for reads if needed**.
...
3. Track liveness/state during execution:
   ...
   ```
4. Coordinator writes progress (sub-agents never call `bd` writes directly):
...
Do NOT run bd writes in this workspace; use bd --readonly --sandbox for reads if needed.

...
- Focus: run `cas review_session start --cwd <cwd> --base <base_branch> --json` plus `cas review_session wait --review-thread-id <reviewThreadId> --timeout-ms <timeout_ms> --json` against the frozen review context as a fixer-owned analysis lens before `P1`, then normalize CAS `reviewResult` fields back into local bookkeeping.
...
6. Determine review-loop activation:
   ...
   - Otherwise verify `comparison_sha` with `git rev-parse --verify <comparison_sha>^{commit}` and freeze the canonical commit for the rest of the run and every outer cycle. For base-branch review runs, also freeze `review_start_cmd=cas review_session start --cwd <cwd> --base <base_branch> --json`, `review_wait_timeout_ms=<timeout_ms>`, `review_wait_timeout_ladder_ms=60000,120000,300000,600000`, `review_wait_budget_ms=1080000`, and the per-run detached-review attempt budget for the frozen review target so later CLI rounds can re-derive the same merge base before invoking the reviewer.
...
### 1) Diff review loop
This phase runs only as the final closure gate on the current final diff after self-review/post-self-review edits settle. `P0 Core Review` reuses CAS `reviewResult` inside `Core passes`, but it is fixer-owned and reported in `Pass trace`, not `Review loop trace`.
...
- Default git-backed review start command: `cas review_session start --cwd <cwd> --base <base_branch> --json`.
- Default git-backed review wait command: `cas review_session wait --review-thread-id <reviewThreadId> --timeout-ms <timeout_ms> --json`.
- Explicit commit-scoped review start command: `cas review_session start --cwd <cwd> --commit <sha> --json`.
- Explicit commit-scoped review wait command: `cas review_session wait --review-thread-id <reviewThreadId> --timeout-ms <timeout_ms> --json`.
- The `start` command resolves the merge base internally for `--base <base_branch>` runs; before each round, confirm that the currently derived merge base still matches the frozen `comparison_sha`.
...
2. Run `cas review_session start --cwd <cwd> --base <base_branch> --json` in a fresh CAS invocation after confirming the live merge base still matches the frozen `comparison_sha`.
3. Capture `reviewThreadId`, `resolvedCodexPath`, and `resolvedCodexVersion` from the `start` JSON response and derive `cas_attempt_key=<review_target_kind>|<comparison_sha>|<resolvedCodexPath>|<resolvedCodexVersion>`.
4. If `start` exits with an allowed fallback `failureCode`, run one native `codex review` round on the same frozen review context, record `review_transport=native_fallback` and `fallback_reason=<failureCode>`, and use the native reviewer output as the round verdict.
5. Otherwise run `cas review_session wait --review-thread-id <reviewThreadId> --timeout-ms <timeout_ms> --json`.
6. If `wait` exits with `failureCode=wait_timed_out`, keep reissuing `wait` on the same `reviewThreadId` using the next rung in the frozen timeout ladder (`60000`, `120000`, `300000`, `600000`) until a verdict arrives or the cumulative `review_wait_budget_ms=1080000` is exhausted; only then stop blocked.
7. If `wait` exits with an allowed fallback `failureCode`, run one native `codex review` round on the same frozen review context, record `review_transport=native_fallback` and `fallback_reason=<failureCode>`, and use the native reviewer output as the round verdict.
8. If native fallback is active on a standard branch-diff review context, keep that fallback on the same frozen whole diff and continue polling/waiting up to the same cumulative `review_wait_budget_ms=1080000`; do not narrow the review target to touched files, the current patch, or another ad hoc sub-scope because the branch diff is large.
9. Otherwise read `reviewResult` from the final CAS JSON response and partition `reviewResult.findings` into:
...
10. Emit one `Review loop trace` row per terminal review round.
11. If `local_findings > 0`, address all review findings, re-run the chosen validation signal, then repeat from step 2.
12. If a repeated locally-fixable finding is stale, suppress it from loop continuation and count it under `stale_findings` only when its normalized fingerprint and implicated path set did not change across consecutive rounds and the current proof bundle directly disproves the finding or the implicated diff hunk/form no longer exists.
...
13. Re-check `Surface proof coverage (deterministic)` against the current diff before closing the round.
14. Close the terminal diff review loop only when a review round yields `local_findings=0`, `blocked_findings=0`, `stale_findings=0`, `overall_correctness="patch is correct"`, and every enumerated proof surface is `proved|blocked`. If `review_transport=cas`, closure also requires `reviewResultAvailable=true`.

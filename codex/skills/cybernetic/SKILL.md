...
- straightforward implementation: use `$accretive-implementer`;
- review/fix/closure loop: use `$resolve`, `$review-adjudication`, or `$fixed-point-driver`;
- code simplification only: use `$simplify-and-refactor-code-isomorphically` or `$reduce`;
- final proof/closure only: use `$verification-closure`;

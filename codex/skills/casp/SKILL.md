---
name: casp
description: Run a v2-only Node JSONL proxy that spawns `codex app-server` and exposes an orchestration-friendly stream API. Use to control Codex programmatically (many threads/turns), auto-handle approvals, forward server requests, fail fast deterministically, and mine sessions via `thread/*` APIs using schemas generated by your installed `codex`.
---
